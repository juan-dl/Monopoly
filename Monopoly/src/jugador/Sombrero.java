...
public final class Sombrero extends Avatar
{
    ...
    public Sombrero(Jugador jugador, Tablero tableroActual)
    {
    ...
    private void moverZigZag(int desplazamiento, int origen)
    {
        ...
        do{
            if(lado == origen)
            {
                setUbicacion(casillas.get(4 - lado).get(9-posicion));
                posicion++;
                desplazamiento--;
                lado = 4 - lado;
            }
            ...
            {
                setUbicacion(casillas.get(4 - lado).get(1+posicion));
                posicion++;
                desplazamiento--;
                lado = 4 - lado; 
            }
            ...
            if(desplazamiento != 0 && (getUbicacion().equals(casillas.get(4 - origen).get(1)) || getUbicacion().equals(casillas.get(origen).get(9))))
            {
                desplazamiento--;
    ...
    @Override public void moverEnAvanzado() throws JugadorException
    {
        ...
        if(desplazamiento > 4)
        {
            ...
            switch(lado){
                ...
                case 0:
                    setUbicacion(getTablero().getCasillas().get(1).get(1));
                    moverZigZag(desplazamiento - 1, 1);
                    break;

                case 1:
                    moverZigZag(desplazamiento, 1);
                    break;

                case 2:
                    setUbicacion(getTablero().getCasillas().get(3).get(1)); 
                    moverZigZag(desplazamiento - 1, 3);
                    break;

                case 3:
                    moverZigZag(desplazamiento, 3);
                    break;     
            }
    ...
    {    
        return ("<br>Id: " + getID() + "<br>Tipo: Sombrero <br>Casilla: "+ getUbicacion().getNombre() +"<br>Jugador: "+ getJugador().getNombre() +"<br><br>");
    }
}
